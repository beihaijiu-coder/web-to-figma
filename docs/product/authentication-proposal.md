...
- API 只发放绑定到单一 `task_id`、单一对象键、短时有效的上传/下载授权。Chrome 与 Figma 均直接与对象存储传输；API 不代理场景包字节。
...
1. 官网 Google 登录、Session JWT、`GET /v1/me` 与内部用户 upsert。
2. Neon 中的套餐与免费周额度数据模型、服务端权益检查。
3. Clerk webhook：处理用户删除及必要的身份同步。
4. Chrome 扩展与 Figma 插件接入统一身份。
5. 支付与 Pro 权益 webhook。
